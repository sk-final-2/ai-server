...
next_question_prompt = ChatPromptTemplate.from_messages([
    ("system", """
당신은 면접관입니다. 지원자의 이전 답변을 바탕으로 꼬리질문을 1개 생성하세요.
...
def first_question_node(state: InterviewState) -> InterviewState:
    print("💬 [first_question_node] 시작 질문 확인")
    if not state.questions:
        state.questions.append("자기소개 해보세요")
        state.step = 1
    return state
...
def next_question_node(state: InterviewState) -> InterviewState:
    ...
    if not answer or answer.strip() == "":
        state.questions.append("이전에 말씀하신 내용을 좀 더 구체적으로 설명해주시겠어요?")
        return state

    try:
        prompt = next_question_prompt.format_messages(answer=answer)
        response = llm.invoke(prompt)
        ...
        next_q = "이전에 말씀하신 내용을 좀 더 구체적으로 설명해주시겠어요?"
